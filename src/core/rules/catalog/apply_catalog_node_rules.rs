...
pub fn apply_catalog_node_rules<'a>(
    ...
    catalog
        ...
        .try_fold(Vec::new(), |mut acc, (catalog_node, rule)| -> anyhow::Result<_> {
            ...
            if !should_run_test(manifest_node, rule.includes.as_ref(), rule.excludes.as_ref()) {
                return Ok(acc);
            }
            ...
            let rule_row_result = match &rule.rule {
                ...
                CatalogSpecificRuleConfig::ColumnsNameConvention { convention, data_types, use_database_columns } => {
                    column_name_convention(
                        catalog_node,
                        manifest_node,
                        convention,
                        data_types.as_ref(),
                        *use_database_columns,
                        rule,
                        verbose,
                    )
                }
